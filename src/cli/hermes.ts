...
export function setupHermes(options: HermesSetupOptions = {}): HermesSetupResult {
  ...
  const backupPath = mergeHermesConfig(configPath, localDir);
...
export function inspectHermes(options: HermesSetupOptions = {}): HermesDoctorCheck {
  ...
  const check: Omit<HermesDoctorCheck, "recommendations"> = {
    ...
    nourish_server_configured: /^\s*nourish\s*:/m.test(config) && /wellness-nourish/.test(config),
    package_pinned: config.includes(PINNED_NPM_PACKAGE),
...
export function hermesSkillMarkdown(localDir = "~/.wellness-nourish"): string {
  return [
    ...
    "4. `mcp_nourish_nourish_log_intake` only after explicit save intent",
    "5. `mcp_nourish_nourish_daily_summary` for today's Telegram summary",
    "",
    "## Personal Store",
    "",
...
function mergeHermesConfig(configPath: string, localDir: string): string | undefined {
  const snippet = `mcp_servers:\n${nourishServerBlock(localDir)}\n`;
  ...
  const next = existing.trimEnd().length ? upsertNourishBlock(existing, localDir) : snippet;
...
function upsertNourishBlock(existing: string, localDir: string): string {
  const lines = existing.trimEnd().split("\n");
  const block = nourishServerBlock(localDir).split("\n");
...
function nourishServerBlock(localDir: string): string {
  return [
    "  nourish:",
    "    command: npx",
    "    args:",
